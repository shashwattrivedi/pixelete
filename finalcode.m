...
src.Brightness = 40;
 ...
 pause(3);
 b=getsnapshot(vid);
% b=imread('C:\Users\gstar\Desktop\pixelete\arena.jpg');
[b,rect]=imcrop(b);
...
% end
% crop_y=imcrop(b);
% yellow=colourthresthold(crop_y);

% crop_r=imcrop(b);
close;

red=colourthresthold(b,crop_r,45);
red=noise(red);
% imtool(red);
close;
% crop_b=imcrop(b);
...
blue=colourthresthold(b,crop_b,45);
blue=noise(blue);
...
% crop_g=imcrop(b);
% green=colourthresthold(crop_g);


 c=(~imfill(red,'holes')==1 &~imfill(blue,'holes')==1);
